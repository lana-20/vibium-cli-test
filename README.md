...
The suite covers **36 bugs**. B1–B33 were originally found in vibium v26.3.18 and verified across 23 sites; B34–B35 were found in v26.5.31. B36–B38 are later write-ups kept in `issues/` rather than suite tests — B37 was killed by a fix that landed the day it was drafted, and **B38 was found against `main` source and filed as [#343](https://github.com/VibiumDev/vibium/issues/343)**.
...
| B38 | Medium | P2 | any command reaching the browser (`go`, `screenshot`, `eval`, input, cookies) | BiDi error detail discarded and the error code printed twice — `Message` has no field for BiDi's *sibling* `message`, so `GetError`'s fallback fabricates the detail from the code | [#343](https://github.com/VibiumDev/vibium/issues/343) OPEN — filed 2026-08-10 |
...
(An earlier draft wrongly flagged this as a macOS-only caveat.) Runnable verification for both
platforms ships in [`issues/verify-b35/`](issues/verify-b35/) — each script prints one paste-ready
RESULT BLOCK.
...
| 2026-07-28 | **B35 hardened to filing standard.** Root cause located in source: the flattening is a deliberate path-traversal guard in `agent/handlers.go`, and the CLI inherits it because `screenshot.go` dispatches through `daemonCall("browser_screenshot", …)` — the CLI is treated as an untrusted agent surface. Verified the MCP `--screenshot-dir` override works over stdio JSON-RPC, confirming the asymmetry: the surface needing confinement is configurable, the one that does not is locked. Found the in-tree fix precedent (`internal/api/recording.go` `WriteRecordToFile`, which is why `record stop -o` honours its path) and proved every needed capability already ships — on-demand directory creation, accurate path errors, a no-disk mode. Found the guard is bypassable via a symlink at the destination basename (defence-in-depth gap, not remotely exploitable). Confirmed long-standing, not a regression. Added `issues/verify-b35/` with bash and PowerShell scripts so a Linux/Windows holder can close the platform gap in two minutes |
...
| 2026-08-10 | **B38 filed upstream as [#343](https://github.com/VibiumDev/vibium/issues/343).** BiDi error detail discarded and the error code printed twice: `Message` (`internal/bidi/protocol.go:43-52`) has no field for BiDi's *sibling* `message`, so `encoding/json` drops it; `GetError` (`:70-86`) then targets a nested shape BiDi never sends, so its fallback fabricates the detail by copying the code, and `session.go:244` prints the duplicated pair. **A parse bug, not a formatting bug** — proved by building `main` @ `59e4b4b` three ways: a formatter-only patch still loses the detail, a struct/parse patch restores it. Hardened ×10 with a per-run nonce. Fact-checking the write-up caught two errors that had survived several confident retellings — an *invented* "expected" output string (Chrome actually sends `Invalid URL: not-a-real-url`) and a cookie repro that **succeeds** on a normal page — plus a stale "3 one-liners" heading over 2 commands. Repro assets in `issues/verify-b38/` |
